...
# CREATE RECIPE
@app.post("/api/recipe", tags=["Recipes CUD"])
...
# UPDATE RECIPE
@app.put("/api/recipe/{id}", tags=["Recipes CUD"])
async def update_recipe(id):
  return 0


# DELETE RECIPE
@app.delete("/api/recipe/{id}", tags=["Recipes CUD"])
async def delete_recipe(id: int):
  response = await remove_recipe(id) 
  if response:
    return {"message": f"{id} has been deleted"}
  return HTTPException(status_code=399, detail="Something went wrong")

@app.post("/api/save_recipe/{recipe_id}", tags=["Recipes CUD"])
...
@app.post("/api/unsave_recipe/{recipe_id}", tags=["Recipes CUD"])
...
# UPLOAD IMAGE
@app.post("/api/upload/recipe-image", tags=["Recipes CUD"])
async def create_upload_image(file: UploadFile = File(...)): #current_user = Depends(get_current_user)
...
@app.post("/api/sign_up", tags=["Login"])
...
# AUTH CHECK
@app.get("/api/authcheck", tags=["Login"])
